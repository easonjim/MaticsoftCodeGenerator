...
namespace Maticsoft.BuilderWeb
{
    ...
    public class BuilderWeb : IBuilder.IBuilderWeb
    {
        ...
        public string GetAddAspx()
        {
            ...
            if (NameSpace.Contains("Maticsoft"))//如果为默认命名空间直接返回
            {
                return strclass.ToString();
            }
            else//否则直接替换原始命名空间
        ...
        public string GetListAspx()
        {
            ...
            foreach (ColumnInfo field in Fieldlist)
            {
                ...
                switch (columnType.Trim().ToLower())
                {
                    case "bit":
                    {
                        strclass.AppendSpaceLine(2, "<%--模板列操作快速方法--%><%--<asp:BoundField DataField=\"" + columnName + "\" HeaderText=\"" + deText + "\" SortExpression=\"" + columnName + "\" ItemStyle-HorizontalAlign=\"Center\"  />--%> ");
                        ...
                        strclass.AppendSpaceLine(4, "<%#Eval(\"" + columnName + "\").ToString().ToLower() == \"true\" ? \"是\" : \"否\"%>");
                        strclass.AppendSpaceLine(3, "</ItemTemplate>");
        ...
        public string GetAddAspxCs()
        {
            ...
            strclass.AppendSpaceLine(3, "Maticsoft.Common.MessageBox.ShowSuccessTip(this,\"保存成功！\",\"add.aspx\");");
            strclass.AppendSpaceLine(3, "/*增加日志*/LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, string.Format(\"Add：【{0}】\", new JavaScriptSerializer().Serialize(model)), this);");
            //return strclass.Value;
            if (NameSpace.Contains("Maticsoft"))//如果为默认命名空间直接返回
            {
                return strclass.ToString();
        ...
        public string GetUpdateAspxCs()
        {
            ...
            strclass.AppendSpaceLine(3, BLLSpace + " bll=new " + BLLSpace + "();");
            //strclass.AppendSpaceLine(3, ModelSpace + " model=new " + ModelSpace + "();");
            strclass.AppendSpaceLine(3, ModelSpace + " model=bll.GetModel(" + Maticsoft.CodeHelper.CodeCommon.GetFieldstrlist(Keys, true) + ");//更新的时候再查询一次，如业务需要可直接new一个实体以减少损耗");
            strclass.AppendSpaceLine(3, "/*增加日志*/LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, string.Format(\"Modify-Old：【{0}】\", new JavaScriptSerializer().Serialize(model)), this);");
            strclass.AppendLine(strclass2.ToString());
            strclass.AppendSpaceLine(3, "bll.Update(model);");
            strclass.AppendSpaceLine(3, "Maticsoft.Common.MessageBox.ShowSuccessTip(this,\"保存成功！\",\"list.aspx\");");
            strclass.AppendSpaceLine(3, "/*增加日志*/LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, string.Format(\"Modify-New：【{0}】\", new JavaScriptSerializer().Serialize(model)), this);");
            //return strclass.ToString();
            if (NameSpace.Contains("Maticsoft"))//如果为默认命名空间直接返回
            {
                return strclass.ToString();
        ...
        public string GetDeleteAspxCs()
        {
            ...
            strclass.AppendSpaceLine(3, "/*增加日志*/LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, string.Format(\"Delete：【{0}】\", new JavaScriptSerializer().Serialize(bll.GetModel(" + _key + "))), this);");
